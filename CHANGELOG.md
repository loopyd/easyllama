...
- Qwen chat no longer crash-loops when it (re)starts while the search group is resident: `--gpu-memory-utilization` is 0.75 rather than 0.85. llama-swap evicts the search group asynchronously, so the chat engine can be asked to start while embeddings, the reranker and the LMCache server still hold VRAM; only ~24.18 GiB of 31.34 GiB is then free, and vLLM's startup gate rejects `util * total > free`, so 0.85 (26.64 GiB) failed engine init with `Free memory on device cuda:0 (24.18/31.34 GiB) on startup is less than desired GPU memory utilization` and every chat request in that state returned 500 (`upstream command exited prematurely`), which wedged callers into retry loops. 0.75 needs 23.5 GiB and fits with margin. The 128K context is unaffected: the KV cache is pinned by `--kv-cache-memory-bytes`, and vLLM logs that it reserves that value and skips memory profiling.
