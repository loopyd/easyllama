...
- Qwen `--gpu-memory-utilization` stays 0.85. 0.75 was tried and reverted: each value fails in a different direction on a 32 GiB card, which is why the chat and the auxiliary models cannot reliably coexist. At 0.85 the aux models load and serve, but a chat start that overlaps an unload fails vLLM's startup gate (`Free memory on device cuda:0 (24.18/31.34 GiB) on startup is less than desired GPU memory utilization (0.85, 26.64 GiB)`), so the engine never initializes and the request returns 500 `upstream command exited prematurely`. At 0.75 the chat starts reliably but leaves too little VRAM for the search group: `/v1/embeddings` and `/v1/rerank` stall and never serve (measured: 400s client timeout, the chat not even evicted, llama-swap logging no start attempt). The 128K context is unaffected either way: the KV cache is pinned by `--kv-cache-memory-bytes` and vLLM skips memory profiling.
