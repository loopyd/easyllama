...
- Qwen `--gpu-memory-utilization` stays 0.85 after three alternatives were measured and rejected. The chat cannot reliably restart while the search group is resident: llama-swap evicts the group asynchronously, so vLLM's synchronous startup gate (`free >= util * 31.34` at engine init) can still see the aux models plus the LMCache server holding ~12.9 GiB and fail with `Free memory on device cuda:0 (24.18/31.34 GiB) on startup is less than desired GPU memory utilization`, which becomes HTTP 500 `upstream command exited prematurely` and wedges callers into retries. 0.79 still fails that gate; 0.75 fails differently (the pinned 4.98 GiB KV cache OOMs: `Tried to allocate 4.98 GiB ... 3.07 GiB free`, and the search group then starves so `/v1/embeddings` and `/v1/rerank` time out after 400s). The `qwen3-embeddings` context was also cut from 131072 to 32768, freeing ~15 GiB of f16 KV in the search group, and that did not close the race either. The aux routes do serve correctly - llama-swap starts, health-checks and evicts them - whenever the chat is not saturated. The 128K chat context is unaffected throughout: the KV cache is pinned by `--kv-cache-memory-bytes`.
