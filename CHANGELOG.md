...
- Qwen `/v1/embeddings` and `/v1/rerank` no longer hang until the client gives up: per-model `ttl: 0` overrides on `qwen3-embeddings` and `qwen3-reranker` stopped llama-swap from ever swapping them in — the proxy log showed no start attempt or health check for either, and neither backend ever received a `/run`. Both overrides were removed so the models inherit the 30-minute `globalTTL`, and the proxy now starts, health-checks and registers them again.
- Qwen mode bounds its model start wait: the `qwen` profile sets `healthCheckTimeout: 300` so a start that cannot become healthy fails observably instead of hanging for the 1800s default. 300s is about 11x the measured warm-cache cold start of the chat backend (27.4s on this machine, 2026-09-20). The global default stays 1800s for profiles whose first start legitimately takes longer (for example the `glm5.3-flash` ~160 GiB checkpoint). Raise the qwen value if you wipe the flashinfer JIT cache, which forces vLLM to recompile kernels on the next start.
- Corrected the record for commit `e24483b`, whose message claimed `healthCheckTimeout 1800 -> 300` and `sendLoadingState` enabled: neither was actually implemented, and the compiled proxy config still read `healthCheckTimeout: 1800` / `sendLoadingState: false`. `sendLoadingState` deliberately stays `false` per the v0.3.2 decision to keep llama-swap loading and switching messages out of client reasoning and context, so only the bound above is implemented. A `sendLoadingState: true` change would need to be argued on its own merits, not smuggled in as an observability fix.
- Qwen `--gpu-memory-utilization` is 0.79 and the `qwen3-embeddings` context is 32768: the measured combination that makes the llama-swap-owned swap work on a 32 GiB card. vLLM's startup gate requires `free >= util * 31.34` at engine init, and the LMCache server keeps the chat's KV pool registered (~6.3 GiB) across swaps, so at most ~25.0 GiB is ever free and util must be <= ~0.799 - at 0.85 a chat restart failed with `Free memory on device cuda:0 (24.18/31.34 GiB) on startup is less than desired GPU memory utilization` and returned HTTP 500 `upstream command exited prematurely`. It cannot go lower either: at 0.75 the pinned 4.98 GiB KV cache failed to allocate (`Tried to allocate 4.98 GiB ... 3.07 GiB free`) and the search group starved. The `qwen3-embeddings` context was cut 131072 -> 32768 because at f16 KV it held ~15 GiB of the card for a server that only ever sees small chunks; that freed ~11 GiB (search-group residency 23.6 -> 7.2 GiB) so the search group no longer pushes the chat over the gate. Verified: chat/embeddings/rerank each 200, llama-swap starts and evicts the aux group, a second chat restart succeeds (26.6s), and 0 engine-init gate or OOM errors. The 128K chat context is unaffected: the KV cache is pinned by `--kv-cache-memory-bytes`.
