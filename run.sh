...
cfg(){ HOST_TZ="${LLAMACPP_HOST_TZ:-$(get_tz)}"; MODELS_DIR="$(apath "${MODELS_DIR}")"; CHAT_TEMPLATE_DIR="$(apath "${CHAT_TEMPLATE_DIR}")"; MMPROJ_DIR="$(apath "${MMPROJ_DIR}")"; [[ -n "${LS_CONFIG_FILE}" ]] && LS_CONFIG_FILE="$(apath "${LS_CONFIG_FILE}")"; }
...
resolve_ls_config(){ local f; if [[ -n "${LS_CONFIG_FILE}" ]]; then [[ -r "${LS_CONFIG_FILE}" ]] || die "no llama-swap config found at ${LS_CONFIG_FILE}; set LLAMACPP_LS_CONFIG_FILE to a readable file"; printf '%s' "${LS_CONFIG_FILE}"; return; fi; for f in "${DEFAULT_LS_CONFIG_FILE}" "${LS_CONFIG_EXAMPLE_FILE}" "${LEGACY_LS_CONFIG_FILE}" "${LEGACY_LS_CONFIG_EXAMPLE_FILE}"; do [[ -r "${f}" ]] || continue; case "${f}" in "${LS_CONFIG_EXAMPLE_FILE}") info "using ${LS_CONFIG_EXAMPLE_FILE}; create ${DEFAULT_LS_CONFIG_FILE} for local overrides" ;; "${LEGACY_LS_CONFIG_FILE}"|"${LEGACY_LS_CONFIG_EXAMPLE_FILE}") warn "using legacy config path ${f}; rename to config.yml/config.yml.example to match the current layout" ;; esac; printf '%s' "${f}"; return; done; die "no llama-swap config found; set LLAMACPP_LS_CONFIG_FILE or create ${DEFAULT_LS_CONFIG_FILE}"; }
