...
cfg_path(){ local f b; f="${LS_CONFIG_FILE}"; [[ -r "${f}" ]] || die "no llama-swap config found at ${f}; set LLAMACPP_LS_CONFIG_FILE or create config.yaml"; b="$(basename "${f}")"; printf '%s|%s' "${f}" "/app/config.d/${b}"; }
